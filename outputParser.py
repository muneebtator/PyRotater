...
# Each element in the list is is a single source population set with sources separated by a newline character ('\n')
...
for filename in os.listdir('outputs'): # The listdir function returns a list with the filenaes and the fort statement loops over them.
    
    if filename.endswith(".txt"): # Check if the model text files exist.
        
        filepath = os.path.join("outputs", filename) # This function creates a complete filepath to the model file.
            ...
            if len(std_errors) != len(source_populations):
                ...
                # Check if index is within the bounds of source_populations
                    if index < len(source_populations):
                        # Iterate over each value in the sublist at the missing index
                        for value in source_populations[index]:
                        # Check if the value is not in the left_pops
                            if value not in left_pops:
                                left_pops.insert(index + 1, "")
                                extra_values.insert(index + 1, "")
                                std_errors.insert(index + 1, "")
                                best_coefficients.insert(index + 1, "")
